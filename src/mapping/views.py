    ...
    try:
        if request.method == "POST":
            ...
            if area_modelled is None:
                ...
                d_focal, sw, sh, iw,ih,gsd,o = 0.012, 0.0131328, 0.0087552, 3840, 2160, 0.025, 0.317
                ...
                ylen1=distance(p1["latitude"],p1["longitude"],p2["latitude"],p2["longitude"])
                ylen2=distance(p3["latitude"],p3["longitude"],p4["latitude"],p4["longitude"])
                ymax = max(ylen1,ylen2)

                xlen1=distance(p1["latitude"],p1["longitude"],p4["latitude"],p4["longitude"])
                xlen2=distance(p2["latitude"],p2["longitude"],p3["latitude"],p3["longitude"])
                ...
                for i in range(xcount * ycount):
                    newx = (p3x + gridl[i] * (p2x - p3x)) + gridr[i] * (p4x - p3x + gridl[i] * (p1x - p2x + p3x - p4x))
                    newy = (p3y + gridl[i] * (p2y - p3y)) + gridr[i] * (p4y - p3y + gridl[i] * (p1y - p2y + p3y - p4y))
                    final_grid.append([newx, newy])
                
                points = MappingRoute(
                points_on_route=json.dumps(
                    final_grid
                ),
                altitude=json.dumps(alt)
                )
                points.save()
            ...
            points = {"points_on_route": json.loads(route.points_on_route), "altitude":json.loads(route.altitude)}
