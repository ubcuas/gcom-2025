...
def stitch_images_expanded(input_pathes, print_log=False):
    ...
    if print_log:
        print("start")
        print(f"{len(input_pathes)} files found")
    ...
    RESULT_SCALE = (
        stitching.images.Images.Resolution.MEDIUM
    )  # to stay under the 2^15 size limit
    ...
    final_imgs = list(images.resize(RESULT_SCALE))
    if print_log:
    ...
    subsetter = stitching.subsetter.Subsetter(confidence_threshold=CONFIDENCE_THRESHOLD)
    ...
    camera_adjuster = stitching.camera_adjuster.CameraAdjuster(
        "reproj", confidence_threshold=CONFIDENCE_THRESHOLD
    )
    ...
    warper = stitching.warper.Warper(warper_type="plane")
    ...
    final_sizes = images.get_scaled_img_sizes(RESULT_SCALE)
    camera_aspect = images.get_ratio(FEATURE_SCALE, RESULT_SCALE)
    ...
    lir_aspect = images.get_ratio(stitching.images.Images.Resolution.LOW, RESULT_SCALE)
