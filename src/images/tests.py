...
class ImageEndpointTests(APITestCase):
    ...
    def setUp(self):
        with self.generate_image_file() as test_image:
            Image.objects.create(image=test_image,
                                 title='Test Image', 
                                 image_type='visible',
                                 taken_at='2021-01-01 12:00:00')
    ...
    def test_post_image(self):
        ...
        self.assertEqual(new_image.taken_at.strftime('%Y-%m-%d %H:%M:%S'), 
                         '2021-01-01 12:00:00')
    ...
    def test_delete_image(self):
        image_count = Image.objects.count()
        with self.generate_image_file() as test_image:
            img = Image.objects.create(image=test_image,
                                 title='Test Image', 
                                 image_type='visible',
                                 taken_at='2021-01-01 12:00:00')
        response = self.client.delete(f'/api/images/{img.id}/', 
                                      format='json')
    ...
    def test_edit_image(self):
        ...
        response = self.client.patch(f'/api/images/{img.id}/', 
                                   {'title': 'New Image',
                                    'image_type': 'thermal',
                                    'taken_at': '2021-01-01 12:00:00'},
                                    format='json')
        ...
        self.assertEqual(updated_image.taken_at.strftime('%Y-%m-%d %H:%M:%S'), 
                         '2021-01-01 12:00:00')
